...
module.exports = {
    ...
    addStatement(settings,  statement) {
      return new Promise(function (resolve, reject) {
        const callApi = require('./callApi');
        const utils = require('./utils');

        let options = {
          url: utils.setAppropriateEnv(`https://platform.cloud.coveo.com/rest/search/v1/admin/pipelines/statements?organizationId=${settings.orgId}`, settings.env),
            method: "POST",
            headers: {
                "Accept": "application/json",
                "Authorization": "Bearer " + settings.apiKey,
        ...
        callApi(options, statement).then(response => {
            if (response.response.statusCode != 201) {
                reject(new CoveoError(response.response.statusCode + ' - ' + response.body, 'addStatement'));
            } else {
                resolve(response);
            }
        }).catch((err) => {
            reject(new CoveoError(err, '[addStatement]'));
        });
    });
  },
    addQueryPipelineStatement(settings, pipelineId, statement) {
        return new Promise(function (resolve, reject) {
          const callApi = require('./callApi');
          const utils = require('./utils');

          let options = {
            url: utils.setAppropriateEnv(`https://platform.cloud.coveo.com/rest/search/v2/admin/pipelines/${pipelineId}/statements?organizationId=${settings.orgId}`, settings.env),
              method: "POST",
              headers: {
                  "Accept": "application/json",
                  "Authorization": "Bearer " + settings.apiKey,
          ...
          callApi(options, statement).then(response => {
              if (response.response.statusCode != 201) {
                  reject(new CoveoError(response.response.statusCode + ' - ' + response.body, 'addQueryPipelineStatement'));
              } else {
                  resolve(response);
              }
          }).catch((err) => {
              reject(new CoveoError(err, '[addQueryPipelineStatement]'));
          });
      });
    },
    addQueryPipelineResultRanking(settings, pipelineId, rank) {
      return new Promise(function (resolve, reject) {
        const callApi = require('./callApi');
        const utils = require('./utils');

        let options = {
          url: utils.setAppropriateEnv(`https://platform.cloud.coveo.com/rest/search/v2/admin/pipelines/${pipelineId}/resultRankings?organizationId=${settings.orgId}`, settings.env),
            method: "POST",
            headers: {
                "Accept": "application/json",
                "Authorization": "Bearer " + settings.apiKey,
        ...
        callApi(options, rank).then(response => {
            if (response.response.statusCode != 200) {
                reject(new CoveoError(response.response.statusCode + ' - ' + response.body, 'addQueryResultRanking'));
            } else {
                resolve(response);
            }
        }).catch((err) => {
            reject(new CoveoError(err, '[addQueryResultRanking]'));
        });
    });
  },
